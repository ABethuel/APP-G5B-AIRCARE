...
$display_topics = $database -> prepare("SELECT * FROM topics") ;
$display_topics -> execute() ;
if ($display_topics -> rowCount()>0){$display_topics = $display_topics->fetchAll();} 
                ...
                <table id="tableUser">
                    ...
                    <?php foreach($display_topics as $dt){?>
                        <tr class="tr_clicks">
                            ...
                            <td class="danger"><form enctype="multipart/form-data" method="post" action="a_deleteTopics.php?id=<?= $dt['id'] ?>"><button class="danger" style="background:none;" href="a_deleteTopics.php?id=<?= $dt['id'] ?>" onclick="if(confirm('Etes-vous sûr de vouloir supprimer le topic ?')){}else{return false;}">Supprimer le Topic</button></form></td>
                            <td><a href="a_response.php?id=<?= $dt['id'] ?>">Voir les messages</a></td>
                        </tr>
                    <?php  } ?>
                    </tbody>
                </table>
